...
-- Seleccionamos toda la tabla transaction y la columna company_name
SELECT transaction.*,companies_C.company_name
FROM transaction
INNER JOIN companies_C  -- Mediante el Join encontramos transacciones de empresas con nombre C
ON companies_C.id = transaction.company_id;
...
LIMIT 1)
-- Query principal para encontrar el nombre de la empresa. Inner Join entre CTE y tabla company
SELECT company_name, compra_mayor
FROM company
INNER JOIN max_transaction
ON company.id = max_transaction.company_id;
...
-- Subquery para averiguar la media de cada país
SELECT country, AVG(amount) AS promedio_país
FROM company
...
FROM transaction
GROUP BY company_id
ORDER BY total_transacciones DESC) transacciones
ON company.id = transacciones.company_id;
